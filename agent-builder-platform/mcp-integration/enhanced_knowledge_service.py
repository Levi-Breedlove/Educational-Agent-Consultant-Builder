...
class EnhancedKnowledgeAccessService:
    ...
    def __init__(self, project_name: str, environment: str = "dev", 
        ...
        try:
            ...
            try:
                ...
                class MockMCPEcosystem:
                    def initialize(self): return True
                    def get_ecosystem_summary(self): return {'total_mcps': 16, 'aws_mcps': 12, 'average_confidence': 0.9, 'capabilities': []}
                    def get_mcp_by_type(self, mcp_type): return None
                
                class MockMCPHealthMonitor:
                    def __init__(self, project_name, environment): pass
                    def is_mcp_healthy(self, mcp_type): return True
                    def get_mcp_status(self, mcp_type): return None
                
                MCPEcosystem = MockMCPEcosystem
                MCPHealthMonitor = MockMCPHealthMonitor
            ...
            for mcp_type, table in tables_to_search:
                try:
                    ...
                    response = table.scan(
                        ...
                        ProjectionExpression='id, #src, content_id, title, content, embedding, embedding_model, last_updated, confidence, metadata',
        ...
        try:
            ...
            for mcp_type, table in tables_to_search:
                try:
                    ...
                    response = table.scan(
                        ProjectionExpression='id, #src, content_id, title, content, last_updated, confidence, metadata',
                        ExpressionAttributeNames={'#src': 'source'}
                    )
    ...
    async def store_knowledge_with_embedding(self, item: KnowledgeItem) -> bool:
        ...
        try:
            ...
            if item.source == "aws":
                table = self.aws_table
            elif item.source == "strands":
                table = self.strands_table
            elif item.source == "mcp":
                table = self.mcp_table
            else:
                logger.error(f"Unknown source: {item.source}")
                return False
    ...
    async def synchronize_mcp_knowledge(self, mcp_type: str, force_sync: bool = False) -> bool:
        ...
        try:
            ...
            # Get MCP configuration (simplified for demo)
            # In production, this would get actual MCP configuration
            if mcp_type not in self.knowledge_tables:
